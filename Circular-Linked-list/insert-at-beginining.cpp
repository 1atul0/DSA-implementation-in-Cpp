...
//insert at begining of circular linked list- O(n) solution
...
//insert at begining of circular linked list O(1) solution
Node* insert_at_begin1(Node* head,int value){
    Node* temp=new Node(value);
     if(head==NULL){
        temp->next=temp;
        return temp;
...
int main()
{
    ...
    curr->next=head;
    Node* res=insert_at_begin1(head,100);
    traverse(res);
    return 0;
}